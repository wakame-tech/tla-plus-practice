...
macro request(data) begin
    query[self] := [type |-> "request", data |-> data] @@ data;
end macro;
...
\* BEGIN TRANSLATION (chksum(pcal) = "5f79b98e" /\ chksum(tla) = "85099b7e")
...
Request(self) == /\ pc[self] = "Request"
                 /\ \/ /\ query' = [query EXCEPT ![self] = [type |-> "request", data |-> ([request |-> "read"])] @@ ([request |-> "read"])]
                       /\ pc' = [pc EXCEPT ![self] = "Confirm"]
                    \/ /\ \E d \in Data:
                            query' = [query EXCEPT ![self] = [type |-> "request", data |-> ([request |-> "write", data |-> d])] @@ ([request |-> "write", data |-> d])]
...
Confirm(self) == /\ pc[self] = "Confirm"
                 ...
                 /\ Assert(result'[self] = ghost_db_history[self], 
                           "Failure of assertion at line 35, column 21.")
